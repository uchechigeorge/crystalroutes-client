...
const LOCAL_HOST = "http://localhost:55042";
const REMOTE_HOST = "https://test.crystalroutes.profeworld.com";
...
export class ApiService {
  ...
  async login(body: { username: string, password: string, deviceId?: string, deviceModel?: string }) {
  ...
  async resetPassword(body: { oldPassword: string, newPassword?: string }) {
    return this.http.post(resetPasswordApiRoute, JSON.stringify(body),{ headers: (await this.getHeaders()) }).toPromise();
  }
  
  async addAdmin(body: { username: string, password?: string }) {
    return this.http.post(addAdminApiRoute, JSON.stringify(body),{ headers: (await this.getHeaders()) }).toPromise();
  }
  
  async sessionsLogin() {
    return this.http.get(sessionsLoginApiRoute,{ headers: (await this.getHeaders()) }).toPromise();
  }

  async logOutDevice(body: { deviceId: string }) {
    return this.http.post(logOutDeviceApiRoute, JSON.stringify(body),{ headers: (await this.getHeaders()) }).toPromise();
  }
  
  async deleteDevice(body: { deviceId: string }) {
    return this.http.post(deleteDeviceApiRoute, JSON.stringify(body),{ headers: (await this.getHeaders()) }).toPromise();
  }
  
  async deleteTracking(body: { trackingId: string }) {
    return this.http.post(deleteTrackingApiRoute, JSON.stringify(body),{ headers: (await this.getHeaders()) }).toPromise();
  }
  
  async deleteTrackingActivity(body: { trackingActivityId: string }) {
    return this.http.post(deleteTrackingActivityApiRoute, JSON.stringify(body), { headers: (await this.getHeaders()) }).toPromise();
  }
  ...
  async viewAdmins(params: {
    ...
    return this.http.get(viewAdminsApiRoute, { params, headers: (await this.getHeaders() ) }).toPromise();
  }
  ...
  async updateTracking(body: {
    ...
    return this.http.post(updateTrackingApiRoute, JSON.stringify(body), { headers: (await this.getHeaders())}).toPromise();
  }
  
  async updateTrackingActivity(body: {
    trackingActivityId?:string,
    description?: string,
    location?: string,
    awaitingLocation?: string
  } | any) {
    return this.http.post(updateTrackingActivityApiRoute, JSON.stringify(body), { headers: (await this.getHeaders())}).toPromise();
  }

  async addTrackingActivity(body: {
    trackingId?:string,
    description?: string,
    location?: string,
    awaitingLocation?: string,
    ...
    return this.http.post(addTrackingActivityApiRoute, JSON.stringify(body), { headers: (await this.getHeaders())}).toPromise();
  }

  async addTracking(body: {
    trackingCode?:string,
    shippingDate?:string,
    deliveryDate?:string,
    hasDelivery?:boolean,
    ...
    return this.http.post(addTrackingApiRoute, JSON.stringify(body), { headers: (await this.getHeaders())}).toPromise();
  }
